...
export default function HomeScreen() {
	...
	const [selectedDate, setSelectedDate] = useState({ year: 2025, month: 4, day: 1 });

	const isPrevMonthAllowed = displayedMonth > 4;
	const isNextMonthAllowed = displayedMonth < 5;

	const weeksByMonth: { [key: string]: (number | null)[][] } = {
		'2025-4': [
	...
	const weeks: (number | null)[][] = weeksByMonth[`${displayedYear}-${displayedMonth}`] || [];
	...
	return (
		<SafeAreaView style={styles.container}>
			<View style={styles.header}>
				<Text style={styles.dateText}>{`${selectedMonthName} ${selectedDay}th, ${selectedYear}`}</Text>
					...
					{weeks.map((week: (number | null)[], wi: number) => (
						<View key={wi} style={{ flexDirection: 'row', width: '100%' }}>
							{week.map((day: number | null, i: number) => {
								...
								const dateId = `${displayedYear}-${String(displayedMonth + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
								const hasEntry = entries.some(e => e.id === dateId);
								const isSelected = selectedDate.year === displayedYear && selectedDate.month === displayedMonth && selectedDate.day === day;
								return (
									<TouchableOpacity
										key={i}
										disabled={isFutureDate}
										style={[styles.dateTextCell, isSelected && styles.selectedDay, day === today.getDate() && displayedMonth === today.getMonth() && displayedYear === today.getFullYear() && styles.today, isFutureDate && styles.futureDate]}
			...
			{selectedEntry ? (
				...
				>
					...
					<View>
						...
						<Text style={styles.eventTime}>{selectedEntry.date} | {selectedEntry.dayTime}</Text>
					</View>
				</TouchableOpacity>
			) : (
			...
			<View style={[styles.addEntryBtnContainer, { paddingBottom: 128 }]}>
				<TouchableOpacity style={styles.addEntryBtn} onPress={() => console.log('Add Entry pressed')}>
